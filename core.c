...
void s(int opened, int n, char line[], int symbols)
{
    if(opened == 0){
        ...
        if(n == 0){
            ...
            int cnt = -1;
            if (line != NULL) {
            //zistime ci patri datum a typ k nacitanemu
                for(int i = 0; i<symbols; i++){
                        if (line[i] == '.'){
                            nazov++;
                        }
                        ...
                        if (nazov == 6) {
                            cn++;
                            if(line[i+1] == '.') temp_typ[cn] = ' ';
                            else temp_typ[cn] = line[i+1];
                            printf("%c", temp_typ[cn]);
                        }else if (nazov == 8) {
                            cnt++;
                            temp_datum[cnt] = line[i+1];
                            printf("%c", temp_datum[cnt]);
                        }else continue;//printf("%c", line[i]);
                }
